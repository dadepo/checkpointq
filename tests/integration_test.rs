...
pub async fn test_only_non_canonical_results() {
    ...
    let first_mock = ("http://www.good1.com".to_string(), Ok(expected_block_root1.to_string()));
    let second_mock = ("http://www.good2.com".to_string(), Ok(expected_block_root2.to_string()));
    let third_mock = ("http://www.good3.com".to_string(), Ok(expected_block_root3.to_string()));
    let client = MockClient::new(
        vec![
            first_mock.clone(),
            second_mock.clone(),
    ...
    let payload1 = non_canonical_result.get(&first_mock.1.clone().unwrap()).unwrap();
    let payload2 = non_canonical_result.get(&second_mock.1.clone().unwrap()).unwrap();
    let payload3 = non_canonical_result.get(&third_mock.1.clone().unwrap()).unwrap();
    assert_eq!(&payload1.get(0).unwrap().payload.data.finalized.root, &expected_block_root1.to_string());
    assert_eq!(&payload2.get(0).unwrap().payload.data.finalized.root, &expected_block_root2.to_string());
    assert_eq!(&payload3.get(0).unwrap().payload.data.finalized.root, &expected_block_root3.to_string());
}
...
pub async fn test_only_failure_results() {
    ...
    assert_eq!(&failure_result.get(0).unwrap().payload.to_string(), &error0.to_string());
    assert_eq!(&failure_result.get(1).unwrap().payload.to_string(), &error1.to_string());
    assert_eq!(&failure_result.get(2).unwrap().payload.to_string(), &error2.to_string());
}